...
run_arm() { # $1 = overrides, $2 = tag, $3 = model
  ...
  for i in $(seq 1 "$REPEATS"); do
    ...
    if [ -n "$gates_line" ]; then
      ...
      # dirge-e31n.8: MECHANISM GATE for anything about tool NAMES. Both
      # halves, because they fail differently: `halluc` is a dispatched call
      # rejected with "Tool X not found" (the model is told and can retry),
      # `dropped` is a call written as text and discarded with no result and
      # no error. An alias table, a rename, a suggestion change — none of them
...
END {
  for (mi = 1; mi <= nm; mi++) {
    ...
    # dirge-e31n.8: MECHANISM for anything touching tool NAMES. The two are
    # separate rows because they are separate failures — the first was
