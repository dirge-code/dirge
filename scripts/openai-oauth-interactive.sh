...
The script forces an isolated DIRGE_DATA_DIR, unsets API-key/provider/model env vars
that could mask OAuth fallback, and sets RUSTFLAGS="" so local builds do not require
the mold linker.
