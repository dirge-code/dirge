...
impl InputEditor {
    ...
    pub(crate) fn open_in_external_editor(&mut self) -> Option<CompactString> {
        ...
        // the raw buffer differs (markers expanded). A failed or aborted edit
        // yields `None` and applies nothing, as before.
        if let Some(content) = crate::ui::external_editor::edit_text(&seed, "input")
            && content != seed
        {
