...
| `turn_facts`              | boolean | Carry an `<unresolved_effects>` block into the turn after a tool call whose effect could not be confirmed — a timeout, a cancellation, a failure after the work started. Default: `false`. A tool result is success-or-error text, which says whether the tool reported a problem, not whether anything reached the disk; without this the model can re-run a committed effect (a second push, a doubled append) or assume work it never did. Requires `turn_envelope`, which owns the block it rides in. **Off by default, and measured that way**: on a scenario built to force the condition (a script that appends a line then hangs past the tool deadline), both tested models scored 6/6 correct in *both* arms at n=6 — they already check the file before deciding whether to re-run. Every other metric read `~noise`. The honest reading is that within a run the model can see the timeout in its own transcript, so the block largely restates what is already visible; the case where it carries information the model genuinely lacks is after a real interrupt, when it cannot see its own tool results, and that half is not implemented yet. Turn it on if you want the belt-and-braces; there is no evidence it helps and none that it hurts. |
